...
const iconopozos = L.icon({
  ...
  iconSize: [40, 40],
  iconAnchor: [20, 40],   // Punta del icono
  popupAnchor: [0, -40]  // Mueve el popup hacia arriba
});

// MARCADORES
const markers = lugares.map(lugar =>
  L.marker([lugar.lat, lugar.lng], { icon: iconopozos })
    .addTo(map)
    .bindPopup(`
      <strong>${lugar.popup}</strong><br>
      ${lugar.nombre}
    `)
);
...
;
  ...
  if (window.innerWidth <= 768) {
    document.getElementById("map").scrollIntoView({
      ...
      infoBox.innerHTML = `
        <h3>${lugar.nombre}</h3>
        <div class="dato"><span>Gasto:</span> ${lugar.ficha.gasto}</div>
        <div class="dato"><span>Estatus:</span> ${lugar.ficha.estatus}</div>
        <div class="dato"><span>Estado:</span> ${lugar.ficha.estado}</div>
        <div class="dato"><span>Domicilio:</span> ${lugar.ficha.domicilio}</div>
        `;
      infoBox.classList.remove("hidden");

      // MAPA
      map.flyTo([lugar.lat, lugar.lng], 16, { duration: 0.8 });
